                ...
                {!hasCharacterBeenCreated &&
                    <div className="form-container">
                        ...
                        <form onSubmit={handleSubmit} className="form-container">
                            <label style={{ fontFamily: "headerFont" }}>Character Name</label>
                            <input style={{ fontFamily: "contentFont" }} value={characterName} placeholder="Enter character name" onChange={(e) => setCharacterName(e.target.value)}></input><br />

                            {/* Physical Attributes */}
                            <label style={{ fontFamily: "headerFont" }}>Physical Attributes</label>
                            {physicalAttributes.map((attribute, index) => (
                                <div className="form-group" key={index}>
                                    ...
                                    />
                                    {index > 0 && (
                                        <button style={{ fontFamily: "headerFont", paddingRight: "4%", paddingLeft: "4%" }} type="button" onClick={() => handlePhysRemoveAttribute(index)}>
                                            Remove
                                        </button>
                                    )}
                                </div>
                            ))}
                            <button style={{ fontFamily: "headerFont" }} type="button" onClick={handlePhysAddAttribute}>Add Physical Attribute</button>
                            <br />

                            {/* Emotional Attributes */}
                            <label style={{ fontFamily: "headerFont" }}>Emotional Attributes</label>
                            {emotionalAttributes.map((attribute, index) => (
                                <div className="form-group" key={index}>
                                    ...
                                    />
                                    {index > 0 && (
                                        <button style={{ fontFamily: "headerFont" }} type="button" onClick={() => handleEmotionalRemoveAttribute(index)}>
                                            Remove
                                        </button>
                                    )}
                                </div>
                            ))}
                            <button style={{ fontFamily: "headerFont" }} type="button" onClick={handleEmotionalAddAttribute}>Add Emotional Attribute</button>
                            <br />

                            {/* Speech Attributes */}
                            <label style={{ fontFamily: "headerFont" }}>Speech Attributes (i.e. ways of speaking)</label>
                            {speechAttributes.map((attribute, index) => (
                                <div className="form-group" key={index}>
                                    ...
                                    />
                                    {index > 0 && (
                                        <button style={{ fontFamily: "headerFont" }} type="button" onClick={() => handleSpeechRemoveAttribute(index)}>
                                            Remove
                                        </button>
                                    )}
                                </div>
                            ))}
                            <button style={{ fontFamily: "headerFont" }} type="button" onClick={handleSpeechAddAttribute}>Add Speech Attribute</button>
                            <br />

                            {/* Equipment Attributes & Items */}
                            <label style={{ fontFamily: "headerFont" }}>Equipment & Items</label>
                            {equipAttributes.map((attribute, index) => (
                                <div className="form-group" key={index}>
                                    ...
                                    />
                                    {index > 0 && (
                                        <button style={{ fontFamily: "headerFont" }} type="button" onClick={() => handleEquipRemoveAttribute(index)}>
                                            Remove
                                        </button>
                                    )}
                                </div>
                            ))}
                            <button style={{ fontFamily: "headerFont" }} type="button" onClick={handleEquipAddAttribute}>Add Equipment/Item</button>
                            <br />

                            {/* Backstory */}
                            <label style={{ fontFamily: "headerFont" }}>Backstory</label>
                            <textarea style={{ fontFamily: "contentFont" }} value={backstory} placeholder="Enter character backstory..." onChange={(e) => setBackstory(e.target.value)}></textarea><br />
                            <input style={{ fontFamily: "headerFont" }} type="submit" value="Create Character" />
                        </form>
                    </div>
                }
                {hasCharacterBeenCreated &&
                <div style={{ fontFamily: "headerFont", display: "flex", flexDirection: "column", alignItems: "center", marginLeft: "25%", marginRight: "25%" }}>
                    <p>
